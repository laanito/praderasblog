...
| `ainarres-bootstrap` | `ainarres-se-construye-a-si-mismo.md` | `ainarres-builds-itself.md` | done | *AINARRES* (Order 2): the v2 bootstrap — built `ainarres status` as an AINARRES project; capability escalation + independent grok integrator. Hero `ainarres-02-bootstrap-hero.webp` (ComfyUI SDXL, seed `27062026`) + `-social.jpg`. |
| `ainarres-hands-off` | `ainarres-corre-sin-director.md` | `ainarres-runs-with-no-conductor.md` | done | *AINARRES* (Order 3): the v3 hands-off run — director removed from the loop; first clean fully-autonomous real-harness run shipped `ainarres ttl` to main (substrate isolation + automatic capability escalation + tiered terminating driver; the two failed runs were test-suite board pollution). Hero `ainarres-03-hands-off-hero.webp` (ComfyUI SDXL, seed `30062026`) + `-social.jpg`. |
| `ainarres-swarm` | `ainarres-en-enjambre.md` | `ainarres-the-swarm.md` | **hero pending** | *AINARRES* (Order 4): v4 — pipeline → swarm; the M18 gate passed (3 concurrent isolated implementers built a real multi-task feature to a coherent main via a single-integrator merge queue). Leads on the live resilience moment — a worker hung on a free-API rate limit, was killed, the substrate released the orphaned claim and a peer finished it (no human routing). Honest arc: first run collapsed on opencode's shared session DB. Gate reframed: concurrency *correctness*, not single-host wall-clock (north star = location-independent). Hero `ainarres-04-swarm-hero.webp` (ComfyUI SDXL, seed `01072026`) + `-social.jpg` — **TO GENERATE**. |
| `openterminalui-intro-de-india` | `openterminalui-bifurcar-terminal-financiero-de-india.md` | `openterminalui-forking-a-financial-terminal.md` | done | New series *OpenTerminalUI* (Order 1): premise + de-India data layer. Hero `openterminalui-01-de-india-intro-hero.webp` (ComfyUI SDXL, seed `24062026`) + `-social.jpg`. |
| `openterminalui-stable-1-0` | `openterminalui-lanzar-1-0-cuando-la-integridad-es-la-funcion.md` | `openterminalui-shipping-1-0-where-integrity-is-the-feature.md` | done | *OpenTerminalUI* (Order 4): the v1.0.0 stable milestone — a hardening/integrity release (silent-mock sweep + degraded-data convention, robustness, single version contract, honest limitations docs). Hero `openterminalui-04-stable-1-0-hero.webp` (ComfyUI SDXL, seed `29062026`) + `-social.jpg`. |
| `openterminalui-portfolio-real` | `openterminalui-la-cartera-se-vuelve-real.md` | `openterminalui-portfolio-becomes-real.md` | done | *OpenTerminalUI* (Order 5): the v1.1 "portfolio becomes real" milestone — cash derived from the ledger (single source of truth), realized P&L fixed from proceeds to true capital gains, and the discovery that the "legacy" portfolio was global/shared (privacy leak) → reachability + import-from-legacy on-ramp. Hero `openterminalui-05-portfolio-real-hero.webp` (ComfyUI SDXL, seed `02072026`) + `-social.jpg`. |
| `openterminalui-retire-shared-portfolio` | `openterminalui-retirar-la-cartera-compartida.md` | `openterminalui-retiring-the-shared-portfolio.md` | done | *OpenTerminalUI* (Order 6): retiring the global, user-less legacy portfolio (v1.1.0). Follows #5 — "just delete the table" meant repointing ~13 consumers (dashboards + risk/stress/factor/dividends/reports/AI-plugin) to a per-user primary via one shared adapter, then deleting the model/routes/tax-lot UI (repoint-then-delete split; parity ported to the Manager first; net −855 lines). Hero `openterminalui-06-retire-shared-portfolio-hero.webp` (ComfyUI SDXL, seed `05072026`) + `-social.jpg`. |
| `openterminalui-run-on-any-brain` | `openterminalui-funciona-con-cualquier-cerebro.md` | `openterminalui-runs-on-any-brain.md` | done | *OpenTerminalUI* (Order 3, published out of order): the provider-agnostic LLM client — one small async client speaking the OpenAI chat-completions + embeddings spec, so the terminal runs on Ollama (default, local)/LM Studio/OpenAI/OpenRouter/Groq/vLLM alike. Structured-output ladder (json_schema→json_object→text), health-checked "unavailable" fallback (no fabrication), one client behind insights+sentiment+RAG embeddings; old LM Studio client superseded (back-compat config aliases). Deep-dive owed from #1. Hero `openterminalui-03-any-brain-hero.webp` (ComfyUI SDXL, seed `08072026`) + `-social.jpg`. |
| `openterminalui-wiring-audit` | `openterminalui-la-auditoria-del-cableado.md` | `openterminalui-the-wiring-audit.md` | done | *OpenTerminalUI* (Order 2, published out of order — LAST reserved series gap): the FE↔backend wiring-audit detective story — "33 bugs that were really 5". FE `api/*.ts` called paths the backend never served → silent 404s hidden by graceful empty-state fallback. Method: boot backend, diff all ~331 client calls vs served `/openapi.json` (298 correct), group failures by cause not feature → ~5 patterns (path-layer /v1 drift, an unmounted economics router = all 404, a shadowed literal route). Angle: one grounded pass beats fanning out AI agents when bugs are correlated; verify against the artifact not the assumption. Source PRs #21/#22/#23. Hero `openterminalui-02-wiring-audit-hero.webp` (ComfyUI SDXL, seed `30072026`) + `-social.jpg`. **Series now complete (#1-#6).** |
| `openterminalui-research-interrogates` | `openterminalui-una-investigacion-que-interroga.md` | `openterminalui-research-that-interrogates.md` | done | *OpenTerminalUI* (Order 7): the **v1.2 "research interrogates"** milestone — the active side of the "don't get fooled" north star. An adversarial *Interrogate this Stock/Coin/Index* card: same local LLM as the briefing but the opposite job — a skeptical devil's-advocate that prosecutes the bull case (Bull Narrative / What Would Have To Be True / Bear Case & Base Rates / Already Priced In?), grounded in the user's OWN notes on the ticker AND their semantically-related notes on other tickers (names the recurring bet/bias). Supporting integrity work: crypto+index news made name-aware (`BTC-USD`→"Bitcoin crypto", `^GSPC`→"S&P 500") and per-article LLM sentiment (batched, cached, honest engine tagging). Reindex-on-note-write so fresh notes reach the adversary. "Challenge my thesis" free-text workspace reslotted to v1.3. Source PRs #79-#84. Hero `openterminalui-07-research-interrogates-hero.webp` (ComfyUI SDXL, seed `31072026`) + `-social.jpg`. |
| `openterminalui-swapping-brains` | `openterminalui-cambiar-de-cerebro.md` | `openterminalui-swapping-brains.md` | done | *OpenTerminalUI* (Order 8): reality-check sequel to #3. Actually swapped the LLM four times (gpt-oss cloud, lfm2, qwen3 mlx, nemotron-nano cloud) and the "runs on any brain" abstraction leaked twice. Leak #1: some endpoints (gpt-oss on Ollama) return HTTP 200 but IGNORE `response_format`, emitting prose → parse fails → feature shows "unavailable"; the ladder only stepped down on error codes, not a 200-with-prose. Fix: carry the required JSON schema IN THE PROMPT, not just the protocol flag. Leak #2: reasoning models (qwen3) burn the whole token budget on hidden `<think>` and return empty/truncated JSON (`finish_reason=length`); fix: one roomier retry when a structured call truncates. Lesson: portability is proven by swapping, not by design; a 200 isn't success, parsing is. Nemotron's initial failure was an honest one (model not pulled → Ollama 404), correctly reported. Source PR #90. Hero `openterminalui-08-swapping-brains-hero.webp` (ComfyUI SDXL, seed `29082026`) + `-social.jpg`. |
| `openterminalui-repository-memory` | `openterminalui-la-memoria-no-debe-vivir-en-el-agente.md` | `openterminalui-memory-should-not-live-in-the-agent.md` | done | *OpenTerminalUI* (Order 9): first-person handoff retrospective after v1.2. A new model joined without Claude's conversational memory, reconciled `.agents` with code, narrowed the release boundary, and completed the asset-aware AI + News hub sentiment + release sequence under human review. Lesson: continuity belongs in repository-owned context, small verifiable PRs, executable tests, and explicit human judgement—not one agent's private memory. Source PRs #92-#95 and release v1.2.0. Dedicated hero `openterminalui-09-repository-memory-hero.webp` + `-social.jpg`. |
| `openterminalui-second-brain-depth` | `openterminalui-cuando-un-segundo-cerebro-aprende-a-crecer.md` | `openterminalui-when-a-second-brain-learns-to-grow.md` | done | *OpenTerminalUI* (Order 10): v1.3 retrospective on making the second brain trustworthy as it grows—stable overlapping chunks, visible retrieval scope, honest streaming fallback, narrow authenticated/idempotent Hermes note ingestion, and deterministic journal gap review that preserves absence instead of inventing it. Wider MCP remains deferred. Source PRs #96-#102 and release v1.3.0. Dedicated hero `openterminalui-10-second-brain-depth-hero.webp` + `-social.jpg`. |
| `openterminalui-surface-truth` | `openterminalui-la-version-que-aprendio-a-decir-no.md` | `openterminalui-the-release-that-learned-to-say-no.md` | done | *OpenTerminalUI* (Order 11): v1.4 Surface Truth retrospective—machine-checked inventory of 86 API families/439 operations, intentional supported/gated/experimental/hidden/remove decisions, owner and compatibility cleanup, and the live-testing sequence that caught stale identity, NSE 403 loops, missing Second Brain capture paths, and malformed/ungrounded AI insights. The shared cancellable/streaming LLM lifecycle stays in v1.6. Source PRs #103–#120 and release v1.4.0. Dedicated hero `openterminalui-11-surface-truth-hero.webp` + `-social.jpg`. |
| `openterminalui-fork-consistency` | `openterminalui-el-terminal-aprende-donde-esta.md` | `openterminalui-the-terminal-learns-where-it-stands.md` | done | *OpenTerminalUI* (Order 12): v1.5 Fork Consistency retrospective—canonical fork identity and docs, shared US/NASDAQ generic defaults with intentional India support preserved, provider-authoritative native currency/locale semantics, and the final human-tested labelled Portfolio holding form. Source PRs #122–#126 and release v1.5.0. Dedicated hero `openterminalui-12-fork-consistency-hero.webp` + `-social.jpg`. |
| `openterminalui-stable-baseline` | `openterminalui-el-terminal-aprende-a-demostrarlo.md` | `openterminalui-the-terminal-learns-to-prove-it.md` | done | *OpenTerminalUI* (Order 13): v1.6 Stable Baseline retrospective—deterministic browser journeys, shared cancellable/validated LLM jobs, intentional lazy-loading boundaries, disposable PostgreSQL and clean-install gates, generated API contracts, and the human-found API-key list mismatch that became a regression before release. The follow-up conversation keeps backend multi-currency accounting in v1.7 before v2. Source PRs #128–#139 and release v1.6.0. Dedicated hero `openterminalui-13-stable-baseline-hero.webp` + `-social.jpg`. |
| `openterminalui-accounting-truth` | `openterminalui-una-cartera-no-puede-sumar-monedas.md` | `openterminalui-a-portfolio-cannot-add-currencies.md` | done | *OpenTerminalUI* (Order 14): v1.7 Accounting Truth retrospective—explicit portfolio/native/transaction/fee currencies, traceable current and historical FX, one shared base-currency accounting contract, security-versus-currency attribution, and the host-tested legacy repair path whose first correct-but-crowded UI was replaced by a focused row action. Source PRs #140–#148 and release v1.7.0. Dedicated hero `openterminalui-14-accounting-truth-hero.webp` + `-social.jpg`. |
...
| `tuqan-agentic-lessons-core-modernization-stepping-stone` | `tuqan-agentic-lessons-core-modernization-stepping-stone.md` | `tuqan-agentic-lessons-core-modernization-stepping-stone.md` | done | Part 3: the Twig deprecation during landing page work that triggered a strategic plan update — treating core functionality modernization as a required stepping stone instead of continued patching; new dedicated hero (`Series_Order: 6`). |
| `tuqan-agentic-lessons-twig-upgrade-stepping-stone-execution` | `tuqan-agentic-lessons-twig-upgrade-stepping-stone-execution.md` | `tuqan-agentic-lessons-twig-upgrade-stepping-stone-execution.md` | done | Part 4: First concrete execution of the stepping stone (Twig 1→3 upgrade + immediate real blocker from ancient Former/illuminate + discovery that /main/ was still serving the cloud 404 due to legacy menu code). Added the exact defensive fallback requested by the user. New dedicated hero (`Series_Order: 7`). |
| `tuqan-agentic-lessons-composer-deps-modernization` | `tuqan-agentic-lessons-composer-deps-modernization.md` | `tuqan-agentic-lessons-composer-deps-modernization.md` | done | Part 5: Large coordinated round of composer dependency modernization. Monolog 2, Phroute 2.2, Former 5, jasny/auth v2, conservative Illuminate 8 floor instead of blindly following ^13. Real compatibility fixes between Former 5 and modern Illuminate. New dedicated hero (`Series_Order: 8`). |

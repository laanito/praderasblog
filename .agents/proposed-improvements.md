...
- **Task B) Series / collections** — **shipped** (2026-04-27): optional front matter (`Series`, `Series_Slug`, `Series_Order`), plugin `plugins/60-SeriesCollections.php`, in-post series nav (prev/next/index), and series indexes at `/series` and `/series/<slug>/` via `content/series.md` + `themes/bootstrap-blog/series.twig`. See post `content/blog/reviviendo-praderas-dia-6-series-y-colecciones.md`.

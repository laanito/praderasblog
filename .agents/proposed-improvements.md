...
- **Phase 6 (low–medium):** AI-ready JSON. **→ v1 shipped (2026-05-20):** `70-BlogJson.php`, `/blog.json` + per-post `.json`, `blog-json-api.md`. **Next:** `search.json`, agent fields (`word_count`, `estimated_tokens`, `modified_at` on listings), `/for-ai-agents` discovery page — `phase-5-6-plan.md`. **Editorial:** articles remain human-first (`editorial-guidelines.md`); JSON does not replace explanatory prose.

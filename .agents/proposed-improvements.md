...
- **Replace random placeholder images**
  ...
  - **Planned — archive retrofit:** add **`Image:`** (shared ES/EN path, **WebP-only** in git) to older posts in **priority tiers** (*Reviving Praderas* Days **1–16** first, then series openers, then long tail). Full steps: `.agents/comfyui-cover-images.md` § *Retrofit plan*. **Steady habit:** target **~2 ES/EN pairs per day**, floor **1 pair**, tick rows in **`.agents/retrofit-cover-queue.md`** so the work is never an all-or-nothing spike.
...
- **Phase 6 (low–medium):** AI-ready JSON. **→ v1.1 shipped (2026-05-24):** `search.json` + `/en/search.json`, listing fields `word_count` / `estimated_tokens` / `modified_at`, schema **1.1**; Day 24 ship log. **→ v1 (2026-05-20):** `70-BlogJson.php`, `/blog.json` + per-post `.json`, `blog-json-api.md`. **Next:** `/for-ai-agents` discovery page (v1.2) — `phase-5-6-plan.md`. **Editorial:** articles remain human-first (`editorial-guidelines.md`); JSON does not replace explanatory prose.
